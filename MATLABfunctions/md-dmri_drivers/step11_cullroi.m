...
if exist('hp_fn','var')
    ...
    try
        opts.SelectedVariableNames = {'AcquisitionDateTime';'PathologyLabel';'MolecularLabel'};
        hp_table = readtable(hp_fn,opts);
...
%     'dtd_msddelta';...
%     'dtd_mdiso_bin1'; 'dtd_mdiso_bin2'; ...
%     'dtd_msddelta_bin1'; 'dtd_msddelta_bin2'; ...  
%     'dtd_fractions'};
...
roi_plots_path = fullfile(fileparts(roi_plots_path),'roi_plots',[roi_plots_name '_3slice_hist_fewermaps']);
...
%         data_name_str = ['grade ' num2str(grade) ' ' data_name];
        data_name_str = [pathologylabels{ndata} '; ' data_name];
    else
    ...
    try
        ...
        for nmap = 1:Nmaps
            ...
            if sz(1) ~= 3
                histdat = I;
                histdat_roi = histdat(roi_I);
                quantile_v = quantile(histdat_roi,[.25 .5 .75]);
                quartile3 = quantile_v(3);
                quartile2 = quantile_v(2);
                quartile1 = quantile_v(1);
                iqr = quartile3-quartile1;
                w = 1.5; outlier_down = [quartile1-w*iqr]; outlier_up = [quartile3+w*iqr];
%                 roi_I = all(cat(4,logical(roi_I), histdat<outlier_up, histdat>outlier_down),4);
%                 roi_I = mio_mask_fill(roi_I);
%                 roi_I = mio_mask_expand(roi_I, 1);
%                 roi_I = all(cat(4,logical(roi_I), histdat<outlier_up, histdat>outlier_down),4);
            end
        end
        ...
        position_hist_0   = [fov.x/(fov.x+fov.y)+.05 .2 fov.y/(fov.x+fov.y)-.1 fov.y/(fov.y+fov.z)-.3];
        ...
        for nmap = 1:Nmaps
            ...
            if sz(1) == 3
                ...
                if isfield(clim,p_name)
                    ...
                    I_maxz = I(:,:,roi_maxz);
                    clim_temp = 1.1*quantile(I_maxz(I_maxz>0),.9999,'all')*[0 1];
                end
                edges = linspace(clim_temp(1),clim_temp(2),Nbins);
                ...
                quartile1 = quantile_v(1);
                iqr = quartile3-quartile1;
                outlier_down = [quartile1-1.5*iqr];
                outlier_up = [quartile3+1.5*iqr];
                
                
                [counts,~] = histcounts(histdat_roi,edges);
                hh = histogram(axh_hist,'BinEdges', edges, 'BinCounts', counts);
                ylim = max(counts(2:(end-1)))*[-.1 1.1];
                set(hh,'DisplayStyle','stairs','EdgeColor',lf_color)
%                 plot(axh_hist,[outlier_down quartile1 quartile2 quartile3 outlier_up],zeros(1,5),'+')
            ...
            imagesc(axh_pmap_x,im2d_x/clim_temp(2))
            imagesc(axh_pmap_y,im2d_y/clim_temp(2))
            imagesc(axh_pmap_z,im2d_z/clim_temp(2))
