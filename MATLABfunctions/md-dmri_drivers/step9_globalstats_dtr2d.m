...
%     
%     pmaps_path = fullfile(data_path,case_name,'rwi','pmaps');    
%     bs_path = fullfile(fileparts(pmaps_path),'dtd','bootstraps');
    bs_path = '/Users/daniel/Dropbox/NMRdata/AV4_500/Linn/PEG_water/dtr2d/bootstraps';
    [~,method,~] = fileparts(fileparts(bs_path));
    pmaps_path = fullfile(fileparts(fileparts(bs_path)),'pmaps');
    

%     nii_fn = fullfile(pmaps_path,[nii_name nii_ext]);
    
%     if exist(pmaps_path,'dir') ~= 7, disp([case_name ' pmaps_path missing']), continue, end
...
%             [diso,~,~,~,~,sddelta] = dtd_pars2dpars(dpar,dperp); 
            [dpar,dperp,~,~,w] = dtr2d_4d_m2pars(mfs.m);
            [diso,~,~,~,~,sddelta] = dtd_pars2dpars(dpar,dperp); 
...
%     [I_msddelta,~] = mdm_nii_read(fullfile(pmaps_path,'dtd_msddelta.nii.gz'));
    [I_s0,~] = mdm_nii_read(fullfile(pmaps_path,'dtr2d_s0.nii.gz'));
    [I_mdiso,~] = mdm_nii_read(fullfile(pmaps_path,'dtr2d_mdiso.nii.gz'));
    [I_msddelta,~] = mdm_nii_read(fullfile(pmaps_path,'dtr2d_msddelta.nii.gz'));
...
%     legend(axh_cont,'boxoff','Location','northeast')
    ...
    print(fullfile(fileparts(bs_path),'global_dtd.pdf'),'-loose','-dpdf')
%     return
% %%
% 
%     figure(3), clf
...
%     
%     set(gcf, 'PaperUnits','centimeters','PaperPosition', [0 0 papersize],'PaperSize', papersize); 
%     print(fullfile(fileparts(pmaps_path),['global_histograms.pdf']),'-loose','-dpdf')
%     
%     %%
% %     figure(10), clf
% %     wm_mask = histdat1 > .5;
