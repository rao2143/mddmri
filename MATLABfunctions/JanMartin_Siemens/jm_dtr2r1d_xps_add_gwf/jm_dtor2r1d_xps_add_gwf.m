...
rps.maxomega = 50 * 2 * pi;  % Maximum frequency
...
for ii = 1 : 1 : xps_gwf.n
    ...
    if abs(b_delta - 1) < tol
        ...
        if xps_gwf.te(ii) < 0.04
            gwf_x = gwf_x_lin_ultrashort;
            gwf_y = gwf_y_lin_ultrashort;
            gwf_z = gwf_z_lin_ultrashort;
        % Case 1.2: Short TE
        elseif xps_gwf.te(ii) < 0.06
    ...
    elseif abs(b_delta) < tol
        ...
        if xps_gwf.te(ii) < 0.06
            gwf_x = gwf_x_sph_short;
            gwf_y = gwf_y_sph_short;
            gwf_z = gwf_z_sph_short;
        % Case 2.2: Long TE
        else
            gwf_x = gwf_x_sph;
            gwf_y = gwf_y_sph;
