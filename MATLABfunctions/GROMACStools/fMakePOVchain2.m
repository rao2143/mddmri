function res = fMakePOVchain2(fnam,gro,molnr,chainatoms,residues)
...
for ncount = 1:length(molnr)
    ...
    for m = 1:length(chainatoms)
        index = [index; find(all([strcmp(gro.atom, chainatoms{m}) strcmp(gro.molecule, residues{m}) gro.molnr==n],2))];
    end
