...
for nexp = 1:length(expno)
    ...
    eval(['print -depsc -loose ' DataDir '/' ExpNam '/' num2str(expno(nexp)) '/PRLFig1D'])
    ...
    axh_sparse2D = axes('position',[left bottom width width*11/8.5]);
...
%     ZprojX = sum(Zval,1);
%     ZprojY = sum(Zval,2);
%     axh_ratio2D = axes('position',[left bottom width width*11/8.5]);
%     clevels = max(max(Zval))*clevels_norm;
%     contour(Xval,Yval,Zval,clevels,'k','LineWidth',.5*lw);
...
%     set([axh_ratio2D],'XTick',log10(Dtick),'XTickLabel',DtickLabel,'YTick',[-3:3])
    ...
    ZprojX = sum(Zval,1);
    ZprojY = sum(Zval,2);
    axh_sparseILT2D = axes('position',[left bottom width width*11/8.5]);
    clevels = max(max(Zval))*clevels_norm;
    contour(Xval,Yval,Zval,clevels,'k','LineWidth',.5*lw);
    hold on
    plot(DiagXY,DiagXY,'--k','LineWidth',.5*lw)
    ...
    set([axh_sparse2D; axh_sparseILT2D; axh_ILT2D],'LineWidth',lw,'FontSize',fs,...
    ...
    eval(['print -depsc -loose ' DataDir '/' ExpNam '/' num2str(expno(nexp)) '/PRLFig2D'])
end
