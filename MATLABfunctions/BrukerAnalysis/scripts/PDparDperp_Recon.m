...
for nexp = 1:length(expno)
    ...
    %calculate dummy data
    SNR = 100;
    Funam = 'fDiffVACSYtricomp';
    S0guess = [.3 .3 .3];
    Dguess = [1e-9 5e-10 25e-11];
    DDeltaguess = [0 .98 0]; 
    Pout = [S0guess Dguess DDeltaguess];
    Yout = feval(Funam,Pout,Xin,Xin2);
    S_array = repmat(Yout,[1 AcqDat.Ndir]);
    S_array = S_array + 1/SNR*sum(S0guess)*randn(AcqDat.Nb,AcqDat.Ndir);
    S_PA = sum(S_array,2); S0 = max(S_PA);
    %figure(1), clf, plot(1:AcqDat.Nb,S_PA/S0,'o'), return
...
%         pause(.1)
    ...
    PDparDperp_smooth_mean = mean(PDparDperp_smooth,2);
    PDisoDratio_smooth_mean = mean(PDisoDratio_smooth,2);
