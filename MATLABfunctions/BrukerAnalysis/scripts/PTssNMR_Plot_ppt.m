...
% expno = [32 52 92]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
...
% ExpNam = {'KEMM57_VT23'};
% expno = [105 55 45 62]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
% % % expno = [12:10:32 45 55 62 72 82 95 105 112]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
% % expno = [105 55 32 22  95 12 45 112 82 72 62 2]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
...
% expno = [22 42 82]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
...
% expno = [22 42 152]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205;
...
expno = [14 34 74]; PT_type = 'DPCPINEPT'; ppmmin = -5; ppmmax = 205; Iscaleppmmin = -5; Iscaleppmmax = 100;
...
% 
% % 
% DataDir = '/Users/daniel/Dropbox/NMRdata Archive/AVII500/DT';
% cd(DataDir)
% ExpNam = {'Pstarch_temp2'};
% expno = [25 65 95]; ppmmin = -5; ppmmax = 205; PT_type = 'DPCPINEPT';
% %expno = [35:10:85]; ppmmin = 56; ppmmax = 109;
% 
% DataDir = '/Users/daniel/NMRdata/AVII500/DT';
% cd(DataDir)
...
for ndir = 1:length(ExpNam)
    ...
    for nexp = length(expno):-1:1
        ...
        if exist('Iscaleppmmin') == 0
            Iscaleppmmin = min(DP.Spec.ppm);
            Iscaleppmmax = max(DP.Spec.ppm);
        end
        Iscale_ind = find([DP.Spec.ppm > Iscaleppmmin & DP.Spec.ppm < Iscaleppmmax]);
        ...
        if strcmp(PT_type,'DPCPINEPT')
            ...
            Imax = 1*max([DP.Spec.I(Iscale_ind); CP.Spec.I(Iscale_ind); INEPT.Spec.I(Iscale_ind)]);

            figure(1)
            h = plot(DP.Spec.ppm,DP.Spec.I/Imax/dI + (nexp-1),'k-');
            set(h,'LineWidth',1.5*lw,'Color',.8*[1 1 1])
            hold on

            h = plot(INEPT.Spec.ppm,INEPT.Spec.I/Imax/dI + (nexp-1),'k-');
            set(h,'LineWidth',1*lw,'Color',[1 0 0])

            h = plot(CP.Spec.ppm,CP.Spec.I/Imax/dI + (nexp-1),'k-');
        ...
        elseif strcmp(PT_type,'DPCP')
            ...
            Imax = 1.1*max([DP.Spec.I(Iscale_ind); CP.Spec.I(Iscale_ind)]);
        ...
        elseif strcmp(PT_type,'DP')
            ...
            Imax = 1.1*max([DP.Spec.I(Iscale_ind)]);
...
%         TextString = ['(' num2str(expno(nexp)) ') ' num2str(NMRacqus.te,3) ' K'];
        ...
        text(ppmmax-.01,nexp-1+.15,TextString,'HorizontalAlignment','left','VerticalAlignment','bottom','FontSize',fs)
...
%     set(gcf,'color','none','inverthardcopy','off');
    ...
    eval(['print -dpng -r300 -loose ' num2str(expno(1)) '/ReportFig_ppt'])
