...
ExpNam = 'Yeast_sedimenation_fexsy_170302'; expno = 2;
ExpNam = 'CK_170306'; expno = 94;
ExpNam = 'Sofia_FEXSY_20171219'; expno = 30;
...
TR.min = 1000e-3; TR.max = 1000e-3; TR.N = 1;
...
G_mix.theta = .1*pi/2; G_mix.phi = .1*pi/4;
G_det.theta = pi/2; G_det.phi = .9*pi/4;
TE.min = 2e-3; TE.max = 30e-3; TE.N = 5;
TR.min = 1000e-3; TR.max = 1000e-3; TR.N = 1;

ga = linspace(G_filt.min,G_filt.max,G_filt.N)';
gb = linspace(G_mix.min,G_mix.max,G_mix.N)';
gc = linspace(G_det.min,G_det.max,G_det.N)';
tm = linspace(TM.min,TM.max,TM.N)';
%te = linspace(TE.min,TE.max,TE.N)';
te = logspace(log10(TE.min),log10(TE.max),TE.N)';
tr = flipud(linspace(TR.min,TR.max,TR.N)');

[gc,te] = ndgrid(gc,te);
ga = G_filt.max*ones(G_det.N,TE.N);
gb = G_mix.max*ones(G_det.N,TE.N);
tm = TM.min*ones(G_det.N,TE.N);
tr = TR.max*ones(G_det.N,TE.N);
...
gax = [gax; ga*sin(G_filt.theta)*cos(G_filt.phi)];
gay = [gay; ga*sin(G_filt.theta)*sin(G_filt.phi)];
gaz = [gaz; ga*cos(G_filt.theta)];
gbx = [gbx; gb*sin(G_mix.theta)*cos(G_mix.phi)];
gby = [gby; gb*sin(G_mix.theta)*sin(G_mix.phi)];
gbz = [gbz; gb*cos(G_mix.theta)];
gcx = [gcx; gc*sin(G_det.theta)*cos(G_det.phi)];
gcy = [gcy; gc*sin(G_det.theta)*sin(G_det.phi)];
gcz = [gcz; gc*cos(G_det.theta)];
