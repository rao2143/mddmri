...
%ExpNam = {'Yeast_sedimenation_fexsy_170302'}; expno = 2:2700; expno = 100;
ExpNam = {'CK_170306'}; expno = [27 28 56 57 77 94]; expno = [113:115 118:124]; expno = 121;
...
%             %figure(1), clf, loglog(b,Iarray,'o'), return
            ...
            FitDat.ADC0(1,npeak) = ADC0;
            FitDat.sigma(1,npeak) = sigma;
            FitDat.AXR(1,npeak) = AXR;
    ...
    %%
        ...
        for npeak = 1:PP.Npeaks
            ...
            title({['\delta=' num2str(PP.peakppm(PP.td1start,npeak),3) ' ppm'];
                ['ADC=' num2str(FitDat.ADC0(npeak),2) ' m^2s^-^1'];
                ['\sigma=' num2str(FitDat.sigma(npeak),2) ]
                ['AXR=' num2str(FitDat.AXR(npeak),2) ' s^-^1']},...
                'FontSize',.5*fs)
            ...
            if npeak < PP.Npeaks
                ...
                title({[num2str(PP.peakppm(PP.td1start,npeak),3)];
                    [num2str(FitDat.ADC0(npeak),2)];
                    [num2str(FitDat.sigma(npeak),2)]
                    [num2str(FitDat.AXR(npeak),2)]},...
                    'FontSize',.5*fs)
            end
        end
