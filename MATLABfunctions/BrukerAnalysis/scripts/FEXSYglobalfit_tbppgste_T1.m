...
ExpNam = {'tbppgste_trtmte_test'}; expno = 110;
...
for nexp = 1:length(expno)
    ...
    if any(strcmp(NMRacqus.pulprog,{'DT_tbppgsteT1T2TM'})) == 1
        ...
        PP.X = reshape(b_det,N_filt,N_det,N_mix);
        PP.X = squeeze(PP.X(1,:,1))';
        b = PP.X;
        tmix = reshape(tmix,N_filt,N_det,N_mix);
%         tmix = [0; squeeze(tmix(1,1,:))];
        tmix = squeeze(tmix(1,1,:))';

        for npeak = 1:PP.Npeaks
%             Xin = PP.X(fitpoints);
            Xin = tmix(FitDat.tmixpoints);
            Yin = PP.Ipeak(:,npeak);
