...
%tdim.j = NMRacqus.l1;
tdim.j = NMRacqus.l1 + NMRacqus.l4;
...
if any(strcmp(NMRacqus.pulprog,{'DT_rare2d_wip'}))
    Stemp = S;
    S = zeros(tdim.i,tdim.j);
%     S(:,1) = Stemp(:,1);
%     S(:,tdim.j/2+1) = .5*(Stemp(:,2) + Stemp(:,3));
    S(:,tdim.j/2+1) = Stemp(:,NMRacqus.l4);
    S(:,(1:tdim.j/2)) = Stemp(:,NMRacqus.l4+((tdim.j-1):-2:1)); 
    S(:,(tdim.j/2+2):tdim.j) = Stemp(:,NMRacqus.l4+(2:2:(tdim.j-2)));
%     S(:,(3:tdim.j/2)) = Stemp(:,(tdim.j-1):-2:5); 
%     S(:,(tdim.j/2+2):(tdim.j)) = Stemp(:,4:2:tdim.j);
end
    
% figure(1), clf, plot(1:tdim.i,real(S(:,(tdim.j/2+1))),1:tdim.i,imag(S(:,(tdim.j/2+1)))), return
% figure(1), clf, plot(1:tdim.i,real(S(:,(2:3))),1:tdim.i,imag(S(:,(2:3)))), return
figure(1), clf, plot(1:numel(S),real(S(:))), return
